    ...
    q-card.absolute-center.q-pa-sm(style="width: 30%" class="flex flex-center justify-center q-gutter-y-sm")
      Form(:validation-schema='registerSchema' class="")
        ...
        .q-gutter-y-sm.q-py-sm.text-center(style="max-width: 300px")
          ...
          q-btn(label="Editar cita" color="brown" @click="validateRegisterForm()")
</template>
...
const register = async () => {
  ...
  const response = await store.put('dog_date/', store.dogId + '/',registerFields.value)
...
const getArticle = async () => {
  const dog = await store.get('dog_date/' + store.dogId + '/')
  registerFields.value = await dog
